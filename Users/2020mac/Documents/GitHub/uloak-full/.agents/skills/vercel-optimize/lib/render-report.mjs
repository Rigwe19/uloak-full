...
lines.push(`- Cold-start rate is very low (${(coldPct * 100).toFixed(2)}%) — Fluid Compute or warm-instance reuse is doing its job.`);
  ...
  if (totalReq > 1000) {
    ...
    if (rate < 0.001) {
lines.push(`- 5xx rate is very low (${(rate * 100).toFixed(3)}%) on ${formatNum(totalReq)} requests.`);
}
  }

  if (lines.length === 0) {
lines.push('_(no headline strengths to call out — see the gated table for signals we considered)_');
}
...
lines.push('- No Speed Insights measurements — Core Web Vitals analysis dormant. Wire up Speed Insights to enable LCP/INP/CLS recommendations.');
  ...
  if (isrMetric.collected) {
    ...
    if (isrR.length === 0) {
lines.push('- No ISR activity observed — either the project does not use ISR or no eligible routes had traffic in the window.');
}
  }
  ...
  if (imageMetric.collected) {
    ...
    if (images === 0) {
lines.push('- No image transformations observed — either `next/image` is not used or no images served in the window.');
}
  }

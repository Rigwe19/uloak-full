...
function hasUnsafeBotProtectionObservation(observation) {
  ...
  if (!/\b(?:Bot Protection|BotID|bot_filter|WAF|managed bot rules?)\b/i.test(text)) {
return false;
}
...
return false;
  ...
  if (/\b(?:use cache:\s*remote|unstable_cache|Cache-Control|s-maxage|cacheLife|export const revalidate|checkBotId|BotID)\b/i.test(action)) {
return true;
}

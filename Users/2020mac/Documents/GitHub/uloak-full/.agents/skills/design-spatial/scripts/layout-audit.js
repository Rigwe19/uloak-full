...
return;
 ...
 if(spread<=0.8||spread>10){
return;
}

      c.forEach(e=>{
 if(Math.abs(e.x-med)>0.8) {
nearmiss.push({x:e.x,med,y:e.y,b:e.b,cy:e.cy,off:Math.round(e.x-med)});
} 
});
    });
  }
 ...
 const cont=document.querySelector(opts.space);
    ...
    if(cont){
 const kids=[...cont.children].filter(vis).map(R).sort((a,b)=>a.y-b.y), gaps=[];
...
gaps.push({g:Math.round(g),a:kids[i],b:kids[i+1]});
      ...
      if(gaps.length>=2){
 const v=gaps.map(o=>o.g),mn=v.reduce((s,x)=>s+x,0)/v.length,
...
return;
    ...
    const cs=getComputedStyle(el),fg=rgb(cs.color);
...
return;
    ...
    if(ra < (large?3:4.5)){
const r=R(el);contrast.push({x:r.x,y:r.y,w:r.w,h:r.h,ratio:Math.round(ra*10)/10,large});
}
  });
  ...
  CONTENT.forEach(it=>{
const w=it.w*it.h*dens(it.el);W+=w;Mx+=w*(it.cx+scrollX);My+=w*(it.cy+scrollY);
});
...
svg.appendChild(e);
    ...
    nearmiss.forEach(n=>{
add('line',{x1:n.x,y1:n.y,x2:n.x,y2:n.b,stroke:COL.align,'stroke-width':3});add('text',{x:n.x+8,y:n.cy+4,fill:COL.align,'font-family':'monospace','font-size':13,_t:(n.off>=0?'+':'')+n.off+'px'});
});
    spacing.gaps.forEach(o=>{
const bad=spacing.outliers.includes(o),x=Math.min(o.a.x,o.b.x)-16;add('line',{x1:x,y1:o.a.b,x2:x,y2:o.b.y,stroke:bad?COL.collision:COL.spacing,'stroke-width':bad?3.5:2});add('text',{x:x-7,y:(o.a.b+o.b.y)/2+4,fill:bad?COL.collision:COL.spacing,'font-family':'monospace','font-size':13,'text-anchor':'end',_t:o.g});
});
    contrast.forEach(c=>{
add('rect',{x:c.x-2,y:c.y-2,width:c.w+4,height:c.h+4,fill:'none',stroke:COL.contrast,'stroke-width':2,'stroke-dasharray':'3 3'});add('rect',{x:c.x,y:c.y-19,width:64,height:17,fill:COL.contrast});add('text',{x:c.x+4,y:c.y-6,fill:'#fff','font-family':'monospace','font-size':12,_t:c.ratio+':1 ✗'});
});
    tap.forEach(it=>{
add('rect',{x:it.x,y:it.y,width:it.w,height:it.h,fill:'none',stroke:COL.tap,'stroke-width':2.5});add('text',{x:it.x,y:it.b+13,fill:COL.tap,'font-family':'monospace','font-size':12,_t:Math.round(it.w)+'×'+Math.round(it.h)});
});

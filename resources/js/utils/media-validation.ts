...
const ALLOWED_AUDIO_MIMES = ['audio/mpeg', 'audio/wav', 'audio/ogg', 'audio/webm', 'audio/mp4', 'audio/aac']
...
export function validateImage(file: File): ValidationResult {
  ...
  if (!ALLOWED_IMAGE_MIMES.includes(file.type)) {
    return { valid: false, error: `Unsupported image format: ${file.type || 'unknown'}. Use JPEG, PNG, WebP, or GIF.` }
  }
...
export function validateAudio(file: File): ValidationResult {
  ...
  if (!ALLOWED_AUDIO_MIMES.includes(file.type)) {
    return { valid: false, error: `Unsupported audio format: ${file.type || 'unknown'}. Use MP3, WAV, OGG, or AAC.` }
  }
...
export function validateDocument(file: File): ValidationResult {
  ...
  if (!ALLOWED_DOCUMENT_MIMES.includes(file.type)) {
    return { valid: false, error: `Unsupported document format: ${file.type || 'unknown'}. Use PDF, DOC, DOCX, or TXT.` }
  }
...
export function validateFile(file: File, mediaType: 'photo' | 'video' | 'audio' | 'document'): ValidationResult {
  switch (mediaType) {
    case 'photo':

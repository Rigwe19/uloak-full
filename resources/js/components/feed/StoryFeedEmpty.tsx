...
export default function StoryFeedEmpty({ hasStories, onContribute, contributeLabel }: StoryFeedEmptyProps) {
    return (
        <motion.div
            key="empty"
            layout
        ...
        >
            <div className="flex flex-col items-center justify-center py-20 text-center">
                ...
                <h3 className="mb-2 text-2xl font-bold tracking-tight text-text-primary">
                    {hasStories ? 'No Stories Match This Filter' : 'No Memories Yet'}
                </h3>
                <p className="mx-auto max-w-md text-sm leading-relaxed text-text-muted">
                    {hasStories
                        ...
                        : contributeLabel || 'No memories have been shared yet.'}
                </p>
                {onContribute && (
                    <button
                        onClick={onContribute}
                        className="mt-6 bg-accent-gold hover:bg-accent-gold/80 text-bg-dark font-mono text-xs font-bold py-3 px-6 rounded-xl uppercase tracking-widest transition-all"
                    >

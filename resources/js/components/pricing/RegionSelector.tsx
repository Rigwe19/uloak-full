...
export function RegionSelector({ regions, selectedRegion, onChange, className = '' }: RegionSelectorProps) {
    ...
    useEffect(() => {
        const handleClickOutside = (event: MouseEvent) => {
            if (event.target instanceof Element && !event.target.closest('[data-region-selector]')) {
                setIsOpen(false);
            }
        };
    ...
    return (
        <div data-region-selector className={`relative inline-block ${className}`}>
            ...
            {isOpen && (
                <ul
                    role="listbox"
                    className="absolute top-full left-0 mt-2 w-64 rounded-xl border border-border-subtle bg-surface py-2 shadow-lg z-50 animate-in fade-in-0 zoom-in-95 duration-200"
                >

...
export function EditRoomModal({ isOpen, room, onClose }: EditRoomModalProps) {
    ...
    const [enableCondolenceAttendance, setEnableCondolenceAttendance] =
        useState(false);
    ...
    const handleTributeSongChange = useCallback(
        (e: React.ChangeEvent<HTMLInputElement>) => {
            const file = e.target.files?.[0];
    ...
    const handleSubmit = useCallback(
        async (e: React.FormEvent) => {
            ...
            formData.append(
                ...
                enableCondolenceAttendance ? '1' : '0',
            );
            formData.append(
                'enable_candle_lighting',
                enableCandleLighting ? '1' : '0',
            );
            ...
            formData.append(
                'existing_media_urls',
                JSON.stringify(retainedExistingUrls),
            );
    ...
    const handleRemoveMember = useCallback(
        async (memberId: number) => {
            ...
            const ok = await confirm(
                'Remove this family member? They will lose access to this room.',
            );
    ...
    const handleRegenerateToken = useCallback(
        async (memberId: number) => {
            ...
            router.post(
                `/dashboard/rooms/${room.slug}/members/${memberId}/regenerate-token`,
                {},
                {
                    preserveScroll: true,
                    onSuccess: () => {
                ...
                {activeTab === 'settings' ? (
                    <form onSubmit={handleSubmit} className="space-y-6 p-6">
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows={3}
                                className="w-full resize-none rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-text-primary placeholder-text-muted/50 transition-colors outline-none focus:border-accent-gold/50"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                type="file"
                                accept="audio/*"
                                onChange={handleTributeSongChange}
                                className="hidden"
                            />
                        </div>
                        ...
                        <div className="space-y-3">
                            <label className="flex items-center gap-3">
                                <input
                                    type="checkbox"
                                    checked={enableTributes}
                                    onChange={(e) =>
                                        setEnableTributes(e.target.checked)
                                    }
                            ...
                            <label className="flex items-center gap-3">
                                <input
                                    type="checkbox"
                                    checked={enableCondolenceAttendance}
                                    onChange={(e) =>
                                        setEnableCondolenceAttendance(
                                            e.target.checked,
                                        )
                                    }
                                    className="accent-accent-gold"
                            ...
                            <label className="flex items-center gap-3">
                                <input
                                    type="checkbox"
                                    checked={enableCandleLighting}
                                    onChange={(e) =>
                                        setEnableCandleLighting(
                                            e.target.checked,
                                        )
                                    }
                                    className="accent-accent-gold"
                    ...
                    <div className="space-y-6 p-6">
                        <p className="text-sm text-text-muted">
                            Add family members to give them access to this room.
                            They'll receive a link they can use from any device
                            — no password needed.
                        </p>
                        ...
                        {membersLoading ? (
                            ...
                            <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 py-12 text-center">
                                ...
                                <p className="text-xs text-text-muted/60">
                                    Add someone above to share this room.
                                </p>
                            </div>
                        ) : (
                            <div className="space-y-3">

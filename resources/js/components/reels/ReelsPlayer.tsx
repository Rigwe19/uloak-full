...
export function ReelsPlayer({
    ...
    >(
        new Map(
            videos.map((v) => [
                v.id,
                { count: v.likes_count ?? 0, isLiked: v.is_liked ?? false },
            ]),
        ),
    );
            ...
            <div className="fixed inset-0 z-100 flex flex-col items-center justify-center gap-6 bg-bg-dark p-8">
                ...
                <div className="text-center">
                    <h2 className="mb-2 text-xl font-bold text-text-primary">
                        No video memories yet
                    </h2>
                    <p className="max-w-xs text-sm leading-relaxed text-text-muted">
                        Video stories shared in this room will appear here as a
                        fullscreen feed.
                    </p>
                </div>
                <button
    ...
    const currentLikes = likesState.get(currentVideo.id) ?? {
        count: currentVideo.likes_count ?? 0,
        isLiked: currentVideo.is_liked ?? false,
    };
        ...
        >
            ...
            <ReelsSocialOverlay
                ...
                onLike={() => {
                    ...
                    // Call backend
                    router.post(
                        `/dashboard/stories/${currentVideo.id}/likes`,
                        {},
                        {
                            preserveScroll: true,
                            onSuccess: () => {

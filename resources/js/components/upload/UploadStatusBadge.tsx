...
const statusConfig: Record<string, { icon: typeof Check; color: string; bg: string }> = {
  ...
  uploading: { icon: Loader2, color: 'text-accent-gold', bg: 'bg-accent-gold/10' },
...
export function UploadStatusBadge({ status, size = 'sm' }: UploadStatusBadgeProps) {
  ...
  return (
    ...
    >
      <Icon size={size === 'sm' ? 10 : 12} className={isSpinning ? 'animate-spin' : ''} />
      {status === 'processing' ? 'Processing' : status === 'uploading' ? 'Uploading' : status === 'queued' ? 'Queued' : status === 'ready' ? 'Ready' : status === 'failed' ? 'Failed' : 'Pending'}
    </motion.div>
  )
}

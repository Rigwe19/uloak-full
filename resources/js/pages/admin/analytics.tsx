      ...
      <div className="mx-auto max-w-7xl space-y-6 p-6">
        ...
        {loading ? (
          ...
          >
            ...
            {activeTab === 'processing' && <ProcessingTab data={data} />}
          </motion.div>
        )}
      </div>
    </AdminLayout>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border-subtle bg-surface/50 p-6">
        <h3 className="mb-4 text-sm font-semibold text-text-primary">Views Over Time</h3>
        <ReactEChartsCore
          echarts={echarts}
          option={viewsChart}
          style={{ height: 320 }}
...
return null;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total Views" value={data.total_views} icon={Eye} color="text-blue-400" />
        <StatCard label="Unique Viewers" value={data.unique_viewers} icon={Users} color="text-purple-400" />
        <StatCard label="Watch Time" value={formatDuration(data.total_watch_time)} icon={Clock} color="text-green-400" />
        <StatCard label="Uploads" value={data.uploads} icon={Upload} color="text-amber-400" />
      </div>

      <div className="rounded-xl border border-border-subtle bg-surface/50 p-6">
        <h3 className="mb-4 text-sm font-semibold text-text-primary">Views & Uploads</h3>
        <ReactEChartsCore
          echarts={echarts}
          option={chartOption}
          style={{ height: 320 }}
      ...
      {/* Top Stories Table */}
      <div className="rounded-xl border border-border-subtle bg-surface/50 p-6">
        <h3 className="mb-4 text-sm font-semibold text-text-primary">Top Stories</h3>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {topStories.map((story: any) => (
                <tr key={story.id} className="border-b border-border-subtle/50 text-text-primary">
                  <td className="py-3 pr-4">{story.title}</td>
                  <td className="py-3 pr-4">{story.views.toLocaleString()}</td>
                  <td className="py-3">{formatDuration(story.watch_time)}</td>
                </tr>
              ))}
              {topStories.length === 0 && (
                <tr>
...
return null;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="New Users" value={data.new_users} icon={Users} color="text-indigo-400" />
        <StatCard label="Total Users" value={data.total_users} icon={Users} color="text-blue-400" />
        <StatCard label="Active Users" value={data.active_users} icon={Activity} color="text-green-400" />
        <StatCard label="Sessions" value={data.sessions} icon={Play} color="text-cyan-400" />
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <StatCard label="Guest Viewers" value={data.guest_viewers} icon={Eye} color="text-amber-400" />
      </div>

      <div className="rounded-xl border border-border-subtle bg-surface/50 p-6">
        <h3 className="mb-4 text-sm font-semibold text-text-primary">New Users Over Time</h3>
        <ReactEChartsCore
          echarts={echarts}
          option={chartOption}
          style={{ height: 320 }}
      ...
      <div className="rounded-xl border border-border-subtle bg-surface/50 p-6">
        <h3 className="mb-4 text-sm font-semibold text-text-primary">Top Contributors</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-border-subtle text-text-muted">
            ...
            <tbody>
              {data.top_contributors?.map((c: any) => (
                <tr key={c.id} className="border-b border-border-subtle/50 text-text-primary">
...
return null;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Total Jobs" value={data.total_jobs} icon={Activity} color="text-blue-400" />
        <StatCard label="Successful" value={data.successful} icon={CheckCircle2} color="text-green-400" />
        <StatCard label="Failed" value={data.failed} icon={XCircle} color="text-red-400" />
        <StatCard label="Avg Duration" value={data.avg_duration_ms ?? 'N/A'} icon={Clock} color="text-amber-400" suffix="ms" />
      </div>

      {Object.keys(failuresByDate).length > 0 && (
        <div className="rounded-xl border border-border-subtle bg-surface/50 p-6">
          <h3 className="mb-4 text-sm font-semibold text-text-primary">Failures Over Time</h3>
          <ReactEChartsCore
            echarts={echarts}
            option={chartOption}
            style={{ height: 320 }}
      ...
      <div className="rounded-xl border border-border-subtle bg-surface/50 p-6">
        <h3 className="mb-4 text-sm font-semibold text-text-primary">Recent Failures</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead>
              <tr className="border-b border-border-subtle text-text-muted">
                <th className="pb-3 pr-4 font-medium">Media</th>
                <th className="pb-3 pr-4 font-medium">Exception</th>
            ...
            <tbody>
              {data.recent_failures?.map((f: any) => (
                <tr key={f.id} className="border-b border-border-subtle/50 text-text-primary">
                  <td className="py-3 pr-4">{f.media_name}</td>
                  <td className="py-3 pr-4 font-mono text-xs text-red-400">{f.exception}</td>
                  <td className="py-3 pr-4">{f.retry_count}</td>
                  <td className="py-3 text-text-muted">{f.created_at}</td>
                </tr>
              ))}

...
import { Briefcase, User, Mail, Lock, AlertCircle, ArrowRight, Eye, EyeClosed, Check, X } from 'lucide-react';
...
function PasswordStrengthMeter({ password }: { password: string }) {
    const requirements = [
        ...
        { id: 'uppercase', label: 'One uppercase letter', test: /[A-Z]/.test(password) },
                ...
                <div className="h-1 w-full rounded-full bg-border-subtle overflow-hidden">
                    <div
                        className={`h-full rounded-full transition-all duration-300 ${
                            satisfiedCount === 0 ? 'bg-red-500' :
                            satisfiedCount === 1 ? 'bg-red-500' :
                            satisfiedCount === 2 ? 'bg-yellow-500' : 'bg-green-500'
                        }`}
                        style={{ width: `${(satisfiedCount / 3) * 100}%` }}
                    />
                </div>
                <p className="text-[10px] text-text-muted">
                    Password requirements: <span className="font-medium">
                        {satisfiedCount === 3 ? 'All satisfied' : `${satisfiedCount} of 3 satisfied`}
                    </span>
                </p>
            </div>
            <ul className="space-y-1">
                {requirements.map((req) => (
                    <li key={req.id} className="flex items-center gap-1.5 text-[10px]">
                        {req.test ? (
                            <Check size={12} className="text-green-500" />
                        ) : (
                            <X size={12} className="text-red-500" />
                        )}
                        <span className={req.test ? 'text-green-400' : 'text-text-muted'}>
                            {req.label}
                        </span>
                    </li>
                ))}
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="flex gap-3">
                                <button
                                    ...
                                    className={`flex flex-1 items-center gap-3 rounded-2xl border px-5 py-3.5 text-sm transition-all ${data.role === 'user'
                                ...
                                >
                                    ...
                                    <div className="flex flex-col items-start">
                                        ...
                                        <span className="text-[10px] text-text-muted">Family legacy & stories</span>
                                    </div>
                                </button>
                                <button
                                    type="button"
                                    onClick={() => setData('role', 'business_admin')}
                                    className={`flex flex-1 items-center gap-3 rounded-2xl border px-5 py-3.5 text-sm transition-all ${data.role === 'business_admin'
                                        ? 'border-accent-gold/50 bg-accent-gold/5 text-accent-gold'
                                        : 'border-border-subtle bg-surface text-text-muted hover:border-accent-gold/30'
                                        }`}
                                >
                                    ...
                                    <div className="flex flex-col items-start">
                                        ...
                                        <span className="text-[10px] text-text-muted">Manage client projects</span>
                                    </div>
                                </button>
                            </div>
                        </div>
                            ...
                            <div className="group relative">
                                ...
                                <div className="relative w-full">
                                    ...
                                    <button
                                        ...
                                        className="absolute right-0 top-0 bottom-0 w-12 flex justify-center items-center text-accent-gold hover:text-accent-gold/70"
                                        tabIndex={-1}
                                    >

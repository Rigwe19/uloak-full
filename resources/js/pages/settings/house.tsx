...
import { User, MessageSquare, Files, Copy, UserPlus, Users, Link as LinkIcon, Share2, Camera, Image, Check, Trash2 } from 'lucide-react';
...
export default function House({ auth, house }: HouseProps) {
    ...
    const [sharingMember, setSharingMember] = useState<HouseMember | null>(null);
    const [thumbPreview, setThumbPreview] = useState<string | null>(house?.thumbnail ?? null);
    const [selectedPattern, setSelectedPattern] = useState(house?.pattern ?? 'none');
    ...
    const handleRemoveMember = useCallback(async (memberId: number) => {
        const ok = await confirm('Remove this house member? They will lose access to all rooms in your house.');
    ...
    const handleRegenerateToken = useCallback(async (memberId: number) => {
        ...
        router.post(`/settings/house/members/${memberId}/regenerate-token`, {}, {
            preserveScroll: true,
            onSuccess: () => {
                loadMembers();
            },
...
return;
        ...
        router.post('/settings/house/pattern', { pattern }, {
            preserveScroll: true,
            ...
            <div className="space-y-8">
                <div>
                    ...
                    <p className="text-sm text-text-muted">
                        Manage house members and their access to your family legacy.
                    </p>
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 space-y-6">
                    <div className="flex items-center gap-2 text-accent-gold">
                        <Image size={16} />
                        <span className="text-xs font-bold tracking-widest uppercase">House Identity</span>
                    </div>
                    <p className="text-xs text-text-muted">
                        Customize the visual identity of this house — a cover thumbnail and background pattern.
                    </p>
                    ...
                    <div className="space-y-3">
                        ...
                        <p className="text-[10px] text-text-muted/60 -mt-1">This image appears as the cover of your house and as preview when sharing the house link.</p>
                        <div
                        ...
                        >
                            {thumbPreview ? (
                                <img src={thumbPreview} className="h-full w-full object-cover" alt="House thumbnail" />
                            ) : (
                                <div className="flex h-full flex-col items-center justify-center gap-2 text-text-muted">
                                    <Camera size={24} />
                                    <span className="text-xs">Upload a house cover image</span>
                                </div>
                            )}
                            <input
                                ref={thumbInputRef}
                    ...
                    <div className="space-y-3">
                        ...
                        <p className="text-[10px] text-text-muted/60 -mt-1">Upload a seamless tile image to use as the background pattern.</p>
                        <div className="flex items-center gap-4">
                            {house?.pattern_upload ? (
                                ...
                                <button
                                    type="button"
                                    onClick={() => patternUploadRef.current?.click()}
                            ...
                            <span className="text-[11px] text-text-muted/70">
                                {house?.pattern_upload ? 'Click the image to remove it.' : 'Click to upload a tileable pattern image.'}
                            </span>
                            <input
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-2">
                            ...
                            {patternSaving && (
                                <span className="text-[9px] text-accent-gold animate-pulse">Saving...</span>
                            )}
                        </div>
                        <p className="text-[10px] text-text-muted/60 -mt-1">A subtle CSS pattern applied as the background of your house.</p>
                        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                            {Object.entries(HOUSE_PATTERNS).map(([key, pattern]) => (
                                <button
                                    key={key}
                                    type="button"
                                    onClick={() => handlePatternChange(key)}
                                    className={`group relative flex flex-col items-center gap-2 rounded-xl border p-4 transition-all ${selectedPattern === key
                                ...
                                >
                                    <div
                                        ...
                                        style={
                                            pattern.preview
                                                ? { background: pattern.preview, backgroundColor: 'rgba(192, 160, 96, 0.03)' }
                                                : { backgroundColor: 'rgba(192, 160, 96, 0.03)' }
                                        }
                                    />
                                    <div className="flex items-center gap-1.5">
                                        <span className={`text-[10px] font-bold tracking-wider uppercase ${selectedPattern === key ? 'text-accent-gold' : 'text-text-muted'
                                            }`}>
                                            {pattern.label}
                                        </span>
                                        {selectedPattern === key && (
                                            <Check size={10} className="text-accent-gold shrink-0" />
                                        )}
                                    </div>
                                </button>
                            ))}
                ...
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 space-y-4">
                    <form onSubmit={handleAddMember} className="space-y-4">
                        <div className="flex items-center gap-2 text-accent-gold">
                            <UserPlus size={16} />
                            <span className="text-xs font-bold tracking-widest uppercase">Add a House Member</span>
                        </div>
                        <p className="text-xs text-text-muted">
                            Add a family member or trusted person to your house. They will be able to view all rooms and create new ones. House members access via a magic link — no password needed.
                        </p>
                        ...
                        <div>
                            <label className="mb-1 block text-[10px] font-bold tracking-wider text-text-muted uppercase">Relationship / Position</label>
                            <p className="mb-2 text-[10px] text-text-muted/60">Used to build the family organogram.</p>
                            <select
                ...
                <div className="space-y-4">
                    ...
                    {membersLoading ? (
                        ...
                        <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-white/10 py-12 text-center">
                            ...
                            <p className="text-sm text-text-muted">No house members added yet.</p>
                            <p className="text-xs text-text-muted/60">Add someone above to share your family house.</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {members.map((member) => (
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                {member.position && (
                                                    <span className="rounded-full border border-accent-gold/20 bg-accent-gold/5 px-2 py-0.5 text-[9px] font-bold tracking-wider text-accent-gold uppercase">
                                                        {FAMILY_POSITIONS.find(p => p.value === member.position)?.label ?? member.position}
                                                    </span>
                                                )}
                                            </div>
